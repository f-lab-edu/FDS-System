...
class RedisDailyTransferAmountCacheAdapter(
    ...
    private val fallbackCounter: Counter = Counter.builder("redis.daily_limit.fallback")
    ...
    @Suppress("unused")
    fun getTodayAmountFallback(userId: Long, ex: Throwable): Long {
    ...
    @Suppress("unused")
    fun addTodayAmountFallback(userId: Long, amount: Long, ex: Throwable): Long {
        fallbackCounter.increment()
        log.warn(
            "[redis-fallback] addTodayAmount userId={} amount={} 실패. 캐시 갱신 누락. cause={}",
            userId, amount, ex.message
        )
        return amount
    }
