...
$$
    BEGIN
        ...
        IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'transfer_outbox_status') THEN
            CREATE TYPE transfer_outbox_status AS ENUM ('PENDING', 'SENDING', 'PUBLISHED', 'DEAD_LETTER');
        END IF;
    END
$$;
...
COMMENT ON COLUMN transfer_events.event_id IS '이벤트 고유 ID (Snowflake) - transaction_id 와 동일';
