...
운영 검증 부재. TPS 측정값은 로컬 단발 결과다. 멀티 인스턴스에서 한 노드가 죽었을 때 복구 시간, Kafka 브로커 한 대 장애 시 Producer 동작, State Store 디스크 풀 상황 모두 직접 안 겪었다. 코드는 동작하지만 실패 시나리오의 동작은 가설이다.

다음으로 테스트 피라미드 위쪽이 비어 있다. contract test, mutation test, chaos test 없음. 회귀 검출이 사람 눈에 의존.

이 문서는 항목이 해소되면 줄을 긋고 새로 발견된 갭을 추가한다.
